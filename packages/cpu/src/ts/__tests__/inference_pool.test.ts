...
const RUN_INFERENCE_POOL_INTEGRATION =
  process.env.ISIDORUS_RUN_INFERENCE_POOL_TESTS === "1";

function isWorkerRuntimeLimitation(err: unknown): boolean {
  ...
  return (
    ...
    msg.includes("Worker init failed")
  );
}

async function createPoolOrSkip(
  t: { skip: (message?: string) => void },
  opts: Parameters<typeof InferencePool.create>[0],
) {
  try {
    return await InferencePool.create(opts);
  } catch (err) {
    if (isWorkerRuntimeLimitation(err)) {
      t.skip(
        "Skipping worker-pool integration test: worker runtime unavailable in current environment",
      );
      return null;
    }
    throw err;
...
describe(
  ...
  {
    skip: !RUN_INFERENCE_POOL_INTEGRATION,
  },
  () => {
    /**
     ...
     */
    it("should perform a real inference using worker-pool and bench_small.pb", async (t) => {
      const pool = await createPoolOrSkip(t, {
     ...
     */
    it("should handle queued requests when all workers are busy", async (t) => {
      const pool = await createPoolOrSkip(t, {
        modelPath: MODEL_PATH,
        inputOp: "Placeholder",
        outputOps: ["StatefulPartitionedCall"],
     ...
     */
    it("should select worker-pool for bench_small.pb (Auto strategy)", async (t) => {
      const pool = await createPoolOrSkip(t, {
        modelPath: MODEL_PATH,
        inputOp: "Placeholder",
        outputOps: ["StatefulPartitionedCall"],
